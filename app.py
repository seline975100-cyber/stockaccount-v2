...
def ai_decision(metrics: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if ma5 and close > ma5:
        score += 1; reasons.append("收盤價站上 MA5，短線動能偏強。")
    elif ma5:
        score -= 1; reasons.append("收盤價低於 MA5，短線需要觀察是否轉弱。")
    if ma20 and close > ma20:
        score += 1; reasons.append("收盤價站上 MA20，中短期趨勢相對穩定。")
    elif ma20:
        score -= 1; reasons.append("收盤價跌破 MA20，波段趨勢偏保守。")
    if ma60 and close > ma60:
        score += 1; reasons.append("收盤價仍在 MA60 之上，中期趨勢尚未明顯破壞。")
    elif ma60:
        score -= 1; reasons.append("收盤價低於 MA60，中期趨勢壓力較大。")
    if rsi is not None:
        if rsi >= 70:
            score -= 1; reasons.append("RSI 高於 70，短線有過熱風險。")
        elif rsi <= 30:
            score += 1; reasons.append("RSI 低於 30，短線可能有跌深反彈機會，但仍需確認止跌。")
        else:
            reasons.append("RSI 位於中性區間，暫無明顯過熱或超跌。")
    if change_pct >= 2:
        reasons.append("單日漲幅較明顯，追價前要觀察量能與隔日延續性。")
    elif change_pct <= -2:
        reasons.append("單日跌幅較明顯，建議先確認是否守住支撐。")
    if score >= 2:
        return {"action": "偏多續抱觀察", "level": "buy", "summary": "技術面分數偏正向，但仍建議分批與設定風險界線。", "reasons": reasons[:5]}
    if score <= -2:
        return {"action": "偏弱減碼控風險", "level": "sell", "summary": "技術面訊號偏弱，應優先觀察支撐與停損區，不宜盲目攤平。", "reasons": reasons[:5]}
    return {"action": "中性觀望", "level": "hold", "summary": "多空訊號尚未明確，適合等待突破或量縮止跌後再調整。", "reasons": reasons[:5]}
...
def ai_analyze_one(code: str, target: date, force_refresh: bool = False) -> Dict[str, Any]:
    code = normalize_code(code)
    cfg = AI_STOCKS.get(code, {"name": NAME_MAP.get(code, code), "keywords": f"{code} 台股 股價 新聞", "focus": []})
    ...
    change_pct = (change / prev_close * 100) if prev_close else 0.0
    lows = [ai_clean_number(r.get("low")) or ai_clean_number(r.get("close")) or 0 for r in usable[-20:]]
    highs = [ai_clean_number(r.get("high")) or ai_clean_number(r.get("close")) or 0 for r in usable[-20:]]
    ma5 = ai_sma(closes, 5)
    ma20 = ai_sma(closes, 20)
    ma60 = ai_sma(closes, 60)
    rsi = ai_rsi(closes, 14)
    support = min([x for x in lows if x > 0], default=close)
    resistance = max([x for x in highs if x > 0], default=close)
    ...
    decision = ai_decision(metrics)
    news, news_source = ai_fetch_news(code, name, str(cfg.get("keywords") or f"{name} {code} 股價"))
    focus = "、".join(cfg.get("focus") or [])
    explanation = f"目前可先看 {metrics['support']} 附近支撐與 {metrics['resistance']} 附近壓力；接下來 1～5 個交易日可搭配量能、外資動向與{focus or '相關新聞'}觀察。"
    return {
        "status": "ok",
        "code": code,
        "name": name,
...
def get_ai_analysis(query_date: str | None, force_refresh: bool = False) -> Dict[str, Any]:
    target = ai_parse_date(query_date)
    codes = list(AI_STOCKS.keys())
    stocks: List[Dict[str, Any]] = []
    # 逐檔抓資料改成平行處理，避免某個資料源逾時拖慢整個頁面。
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(5, len(codes))) as executor:
        future_map = {executor.submit(ai_analyze_one, code, target, force_refresh): code for code in codes}
        for future in concurrent.futures.as_completed(future_map, timeout=35):
            code = future_map[future]
            try:
                stocks.append(future.result())
            except Exception as e:
                cfg = AI_STOCKS.get(code, {})
    ...
    def do_GET(self):
        ...
        if path == "/api/analyze":
            try:
                ...
                data = get_ai_analysis(query_date, force)
