...
def ai_decision(metrics: Dict[str, Any], news: List[Dict[str, str]] | None = None, cfg: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    if rsi is not None:
        if rsi >= 75:
            tech_score -= 2; reasons.append("RSI 高於 75，短線過熱，追高風險大。")
        elif rsi >= 60:
    ...
    if change_pct >= 3:
        reasons.append("單日漲幅較大，若要加碼應分批，不宜一次追滿。")
    elif change_pct <= -3:
        reasons.append("單日跌幅較大，若跌破支撐應先減碼，不建議硬攤平。")
    ...
    support_f = float(support or close or 0)
    resistance_f = float(resistance or close or 0)
    close_f = float(close or 0)
    ma20_f = float(ma20 or close_f or 0)

    add_low = max(0, min(support_f * 1.00, close_f * 0.97))
    add_high = max(add_low, min(close_f * 1.01, max(ma20_f * 1.02, support_f * 1.06)))
    trim_low = max(close_f * 1.04, resistance_f * 0.99)
    trim_high = max(trim_low, resistance_f * 1.06)
    stop_low = max(0, support_f * 0.95)
    stop_high = max(stop_low, support_f * 0.98)
    target_low = max(close_f * 1.03, resistance_f)
    target_high = max(target_low, resistance_f * 1.08)

    if total_score >= 4:
        action = "積極分批加碼"
        level = "buy"
        summary = "技術面與題材面同時偏強，可以大膽一點採取分批加碼，但仍要守停損。若股價在建議加碼區間內且沒有爆量長黑，優先考慮加碼；接近壓力區則分批停利。"
    elif total_score >= 2:
        action = "偏多續抱，拉回加碼"
        level = "buy"
        summary = "多方條件略占優勢，不必急著賣；比較好的做法是續抱，若拉回到加碼區間且未跌破支撐，可分批加碼。"
    elif total_score <= -4:
        action = "明確減碼 / 跌破停損賣出"
        level = "sell"
        summary = "技術面與題材面同步偏弱，這時不要硬凹。若反彈到減碼區間可先降部位；若跌入停損區間，建議果斷賣出控風險。"
    elif total_score <= -2:
        action = "偏弱，反彈減碼"
        level = "sell"
        summary = "目前風險大於機會，若有反彈接近 MA20 或壓力區，建議減碼；除非重新站回 MA20 且新聞面轉強，否則不建議加碼。"
    else:
        action = "區間操作，等突破再加碼"
        level = "hold"
        summary = "多空訊號不夠一致，不建議衝動加碼。較好的策略是在支撐附近小量試單，突破壓力再加碼；跌破停損區間則先退出。"
...
def ai_analyze_one(code: str, target: date, force_refresh: bool = False, names_map: Dict[str, str] | None = None) -> Dict[str, Any]:
    ...
    explanation = f"目前價格大致落在 {decision.get('strategy', {}).get('add_range', '支撐區')} 與 {decision.get('strategy', {}).get('trim_range', '壓力區')} 的操作帶；接下來 1～5 個交易日可搭配量能、外資動向與{focus or '相關題材'}確認。"
    return {
        "status": "ok",
        "code": code,
        "name": name,
