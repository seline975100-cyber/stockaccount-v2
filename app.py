...
EMAIL_IMPORT_TOKEN = os.environ.get("EMAIL_IMPORT_TOKEN", "").strip()
GMAIL_ALLOWED_SENDER = os.environ.get("GMAIL_ALLOWED_SENDER", "").strip().lower()
...
# Gmail 證券成交回報匯入
# ═══════════════════════════════════════════
def html_to_text_for_report(html_body: str, plain_body: str = "") -> str:
    """把 Gmail HTML 表格轉成可解析的純文字。"""
...
def parse_report_date(text: str, fallback: str = "") -> str:
    ...
    if fallback:
        try:
            # Apps Script 會傳 ISO 字串；若不是 ISO，取前 10 碼盡量使用。
            if re.match(r"\d{4}-\d{2}-\d{2}", fallback):
                return fallback[:10]
            return datetime.fromisoformat(fallback.replace("Z", "+00:00")).astimezone(TW_TZ).strftime("%Y-%m-%d")
        except Exception:
            pass
    return datetime.now(TW_TZ).strftime("%Y-%m-%d")


def parse_stock_name_cell(cell: str) -> Tuple[str, str]:
    compact = re.sub(r"\s+", "", str(cell or ""))
    m = re.match(r"(?P<code>\d{4,6})(?P<name>.+)?", compact)
    if not m:
        return "", compact
    return normalize_code(m.group("code")), (m.group("name") or "").strip()


def parse_fubon_execution_rows(text: str) -> List[Dict[str, Any]]:
    """解析富邦證券成交回報表格。可處理 HTML table 轉出的 tab 文字與一般純文字。"""
    rows: List[Dict[str, Any]] = []
    seen = set()

    def add_row(code: str, name: str, trade_type: str, qty: Any, price: Any, amount: Any = "", order_no: str = "", fill_time: str = ""):
        code = normalize_code(code)
        if not code:
            return
        action = "sell" if "賣" in str(trade_type) else "buy"
        qty_n = to_float(qty)
        price_n = to_float(price)
        amount_n = to_float(amount)
        if qty_n <= 0 or price_n <= 0:
            return
        key = (code, action, int(qty_n), round(price_n, 4), str(order_no), str(fill_time))
        if key in seen:
            return
        seen.add(key)
        rows.append({
            "code": code,
            "name": name or NAME_MAP.get(code, code),
            "trade_type": str(trade_type).strip(),
            "action": action,
            "qty": int(qty_n) if float(qty_n).is_integer() else qty_n,
            "price": price_n,
            "amount": amount_n,
            "orderNo": str(order_no or "").strip(),
            "fillTime": str(fill_time or "").strip(),
        })

    for raw_line in text.splitlines():
        line = raw_line.strip()
        if not line or not re.search(r"\d{4}", line) or not re.search(r"現買|現賣|買進|賣出|資買|資賣|券買|券賣", line):
            continue

        if "\t" in line:
            cols = [c.strip() for c in line.split("\t") if c.strip()]
            # 常見欄位：股票名稱、交易類別、成交股數、成交單價、成交價金、委託書編號、成交時間、備註
            if len(cols) >= 4:
                stock_idx = next((i for i, c in enumerate(cols) if re.match(r"\s*\d{4,6}", c)), 0)
                if stock_idx + 3 < len(cols):
                    code, name = parse_stock_name_cell(cols[stock_idx])
                    trade_type = cols[stock_idx + 1] if stock_idx + 1 < len(cols) else ""
                    qty = cols[stock_idx + 2] if stock_idx + 2 < len(cols) else ""
                    price = cols[stock_idx + 3] if stock_idx + 3 < len(cols) else ""
                    amount = cols[stock_idx + 4] if stock_idx + 4 < len(cols) else ""
                    order_no = cols[stock_idx + 5] if stock_idx + 5 < len(cols) else ""
                    fill_time = cols[stock_idx + 6] if stock_idx + 6 < len(cols) else ""
                    add_row(code, name, trade_type, qty, price, amount, order_no, fill_time)
                    continue

        # 沒有 tab 時，用寬鬆 regex 解析。
        compact_line = re.sub(r"\s+", " ", line)
        m = re.search(
            r"(?P<stock>\d{4,6}\s*[\u4e00-\u9fffA-Za-z0-9（）()\-\.]+?)\s+"
            r"(?P<trade>現買|現賣|買進|賣出|資買|資賣|券買|券賣)\s+"
            r"(?P<qty>[\d,]+(?:\.\d+)?)\s+"
            r"(?P<price>[\d,]+(?:\.\d+)?)\s+"
            r"(?P<amount>[\d,]+(?:\.\d+)?)"
            r"(?:\s+(?P<order>\S+))?"
        ...
        if m:
            code, name = parse_stock_name_cell(m.group("stock"))
            add_row(code, name, m.group("trade"), m.group("qty"), m.group("price"), m.group("amount"), m.group("order") or "", m.group("time") or "")
    return rows


def build_gmail_record(row: Dict[str, Any], payload: Dict[str, Any], report_date: str, state: Dict[str, Any], idx: int) -> Dict[str, Any]:
    settings = state.get("settings") if isinstance(state.get("settings"), dict) else {}
    fee_rate = to_float(settings.get("feeRate", 0.1425)) or 0.1425
    qty = to_float(row.get("qty"))
    price = to_float(row.get("price"))
    amount = qty * price
    action = row.get("action") or "buy"
    fee = round(amount * fee_rate / 100)
    tax = round(amount * 0.3 / 100) if action == "sell" else 0
    fill_time = row.get("fillTime") or ""
    order_no = row.get("orderNo") or ""
    note_parts = ["Gmail 富邦證券成交回報自動匯入"]
    if fill_time:
        note_parts.append(f"成交時間 {fill_time}")
    if order_no:
        note_parts.append(f"委託書編號 {order_no}")
    if payload.get("subject"):
        note_parts.append(f"信件：{payload.get('subject')}")
    base_id = int(time.time() * 1000) + idx
    return {
        "id": base_id,
        "date": report_date,
        "code": row.get("code"),
        ...
        "ts": base_id,
        "source": "gmail_fubon_execution_report",
        "sourceMessageId": payload.get("messageId") or payload.get("message_id") or "",
        "brokerOrderNo": order_no,
        "fillTime": fill_time,
        "emailFrom": payload.get("from") or "",
    }


def gmail_record_key(record: Dict[str, Any]) -> Tuple[Any, ...]:
    return (
        record.get("sourceMessageId") or "",
        record.get("brokerOrderNo") or "",
        record.get("date") or "",
        normalize_code(record.get("code") or ""),
        record.get("action") or "",
        str(record.get("qty") or ""),
        str(record.get("price") or ""),
        record.get("fillTime") or "",
    )


def import_gmail_execution_report(payload: Dict[str, Any]) -> Dict[str, Any]:
    sender = str(payload.get("from") or payload.get("sender") or "").lower()
    if GMAIL_ALLOWED_SENDER and GMAIL_ALLOWED_SENDER not in sender:
        return {"ok": False, "error": "寄件者不在允許清單", "sender": sender, "allowed": GMAIL_ALLOWED_SENDER}

    text = html_to_text_for_report(str(payload.get("htmlBody") or payload.get("html") or ""), str(payload.get("plainBody") or payload.get("body") or ""))
    report_date = parse_report_date("\n".join([str(payload.get("subject") or ""), text]), str(payload.get("date") or ""))
    rows = parse_fubon_execution_rows(text)
    if not rows:
        return {"ok": False, "error": "沒有解析到成交回報資料", "subject": payload.get("subject") or "", "preview": text[:800]}

    state = load_app_state()
    existing = {gmail_record_key(r) for r in state.get("records", []) if isinstance(r, dict)}
    imported: List[Dict[str, Any]] = []
    skipped = 0
    for idx, row in enumerate(rows):
        record = build_gmail_record(row, payload, report_date, state, idx)
        key = gmail_record_key(record)
        if key in existing:
            skipped += 1
            continue
...
def ai_build_tomorrow_plan(stocks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for s in stocks or []:
        ...
        plan.append({
            ...
            "buy_range": strategy.get("add_range") or "—",
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_OPTIONS(self):
        ...
        self.send_header("Access-Control-Allow-Headers", "Content-Type, X-Import-Token")
        self.end_headers()
    ...
    def do_POST(self):
        ...
        if path == "/api/gmail/import":
            try:
                token = qs.get("token", [""])[0] or self.headers.get("X-Import-Token", "")
                if not EMAIL_IMPORT_TOKEN:
                    return self._send_json({"ok": False, "error": "Render 尚未設定 EMAIL_IMPORT_TOKEN 環境變數"}, 500)
                if token != EMAIL_IMPORT_TOKEN:
                    return self._send_json({"ok": False, "error": "匯入 token 錯誤"}, 403)
                payload = self._read_json()
                result = import_gmail_execution_report(payload)
                return self._send_json(result, 200 if result.get("ok") else 422)
            except Exception as e:
    ...
    def do_GET(self):
        ...
        if path == "/health":
            ...
            return self._send_json({"ok": True, "version": APP_VERSION, "time": now_tw_text(), "storage": storage_status, "gmail_import": {"token_configured": bool(EMAIL_IMPORT_TOKEN), "allowed_sender": GMAIL_ALLOWED_SENDER}})
